...
export function useGetImage({podcastId}: {podcastId: string}) {
    ...
    useEffect(() => {
        ...
        const fetchImage = async () => {
            const { data: exists } = await supabase.storage.from("podcast-cover-images").exists(`${podcastId}.png`);
            if (!exists) {
            ...
            try {
                const { data } = supabase.storage.from("podcast-cover-images").getPublicUrl(`${podcastId}.png`);
